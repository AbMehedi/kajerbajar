...
const NAV_ITEMS = {
  student: [
    ...
    { href: "/student/learn", label: "Learn & Verify", icon: BookOpen },
  ...
  admin: [
    ...
    { href: "/search", label: "Directory Search", icon: Search },
    { href: "/admin/learning/queue", label: "Learning Queue", icon: BookOpen },
...
const ROLE_BADGE_STYLES = {
  student:
    "border-[hsl(var(--kb-brand-500))/0.35] text-[hsl(var(--kb-brand-400))] bg-[hsl(var(--kb-brand-500))/0.12]",
...
export default function DashboardShell({
  ...
  const roleBadgeClass =
    ROLE_BADGE_STYLES[role] ?? "border-white/20 text-slate-300 bg-white/5";
  ...
  return (
    <div className="min-h-screen bg-[hsl(var(--kb-surface-900))] text-[hsl(var(--kb-text-primary))]">
      ...
      {/* Mobile top bar */}
      <div className="md:hidden sticky top-0 z-40 border-b border-[hsl(var(--kb-border))] bg-[hsl(var(--kb-surface-900))]">
        <div className="flex items-center justify-between px-4 py-3">
          <button
            type="button"
            aria-label="Open sidebar"
            onClick={() => setDrawerOpen(true)}
            className="rounded-md border border-[hsl(var(--kb-border))] bg-black/5 dark:bg-white/5 px-3 py-2 text-sm text-[hsl(var(--kb-text-primary))]"
          >
            Menu
          </button>
      ...
      {/* Desktop sidebar */}
      <aside
        className="hidden md:flex fixed inset-y-0 left-0 z-30 border-r border-[hsl(var(--kb-border))] glass"
        style={{ width: "var(--kb-sidebar-width)" }}
      >
        <div className="flex h-full w-full flex-col px-4 py-6">
          <div className="mb-6">
            <div className="flex items-center justify-between">
              <div className="text-lg font-semibold text-[hsl(var(--kb-text-primary))]">
                কাজের বাজার
              </div>
            </div>
            <div
              className={`mt-3 inline-flex items-center gap-2 rounded-full border px-3 py-1 text-[11px] ${roleBadgeClass}`}
            >
                    ...
                    <motion.div key={item.href} variants={itemVariants}>
                      <Link
                        ...
                        className={
                          ...
                          (isActive
                            ? "border-l-2 border-[hsl(var(--kb-brand-500))] bg-[hsl(var(--kb-brand-500))/0.12] text-[hsl(var(--kb-text-primary))]"
                            : "text-[hsl(var(--kb-text-secondary))] hover:bg-[hsl(var(--kb-surface-700))] hover:text-[hsl(var(--kb-text-primary))]")
                        }
                      >
          ...
          <div className="mt-6 border-t border-[hsl(var(--kb-border))] pt-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-[hsl(var(--kb-text-muted))]">{roleLabel}</div>
              </div>
            </div>
            <div className="mt-4">
              <LogoutButton />
